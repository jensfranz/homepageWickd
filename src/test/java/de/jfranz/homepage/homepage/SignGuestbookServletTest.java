...
package de.jfranz.homepage.homepage;
...
import de.jfranz.homepage.main.SignGuestbookServlet;
...
public class SignGuestbookServletTest {
  ...
  public void testDoPost() throws IOException, EntityNotFoundException {
    ...
    String homepageName = "TestGuestbook";
    String testContent = "Test Content";

    when(request.getParameter("homepageName")).thenReturn(homepageName);
    ...
    verify(response).sendRedirect("/homepage.jsp?homepageName=TestGuestbook");

    User currentUser = UserServiceFactory.getUserService().getCurrentUser();

    Entity greeting = DatastoreServiceFactory.getDatastoreService().prepare(new Query()).asSingleEntity();

    assertEquals(homepageName, greeting.getKey().getParent().getName());
